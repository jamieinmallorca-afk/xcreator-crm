...
async function getDashboardData(profileId: string) {
  ...
  const [subscribersRes, alertsRes] = await Promise.all([
    db
      .from('subscribers')
      .select('health_score, churn_risk, is_active, created_at')
...
export default async function DashboardPage({
  ...
  // Not connected — show landing CTA
  if (!session) {
    return (
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            accent={stats.highRisk > 0 ? 'text-red-400' : 'text-green-400'}
          />
        </div>
          ...
          {topSubs.length === 0 ? (
            <div className="bg-white/5 rounded-xl p-8 text-center text-gray-500">
              ...
              <p className="text-sm mt-1">
                The daily sync runs automatically — or trigger it manually via the API.
              </p>
            </div>
          ) : (
                  ...
                  {topSubs.map((sub, i) => (
                    <tr key={i} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                      <td className="px-4 py-3">
                        ...
                        {sub.x_display_name && (
                          <div className="text-gray-500 text-xs">{sub.x_display_name}</div>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        <span className={`font-bold ${healthColor(sub.health_score ?? 0)}`}>
                          {sub.health_score ?? '—'}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 hidden sm:table-cell text-gray-400">
                        {sub.followed_at
                          ? new Date(sub.followed_at).toLocaleDateString()
                          : '—'}
                      </td>
                    </tr>
                  ))}
